      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-24">
        ...
        <div className="space-y-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {values.map((val, idx) => (
              <div key={idx} className="p-6 rounded-2xl glass border border-white/5 space-y-4 hover:bg-white/[0.04] transition-colors duration-200">
                <div className="w-10 h-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center">
                  {val.icon}
                </div>
                <h3 className="text-lg font-bold text-slate-200 font-heading">{val.title}</h3>
        ...
        {/* Stats Summary */}
        <div className="rounded-3xl bg-gradient-to-tr from-[#6C5CE7]/15 via-transparent to-transparent border border-white/5 p-8 md:p-12">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center divide-y md:divide-y-0 md:divide-x divide-white/5">
            <div className="space-y-2 py-4 md:py-0">
              ...
              <p className="text-sm font-bold text-slate-200 uppercase tracking-wider font-heading">Active Job Openings</p>
              <p className="text-xs text-slate-400">Verifiably updated hourly</p>
            </div>
            <div className="space-y-2 py-4 md:py-0">
              <p className="text-4xl font-extrabold text-[#6C5CE7] font-heading">94%</p>
              <p className="text-sm font-bold text-slate-200 uppercase tracking-wider font-heading">Candidate Success Rate</p>
              <p className="text-xs text-slate-400">Securing screening calls within 14 days</p>
            </div>
            <div className="space-y-2 py-4 md:py-0">
              <p className="text-4xl font-extrabold text-[#00D2D3] font-heading">20+</p>
              <p className="text-sm font-bold text-slate-200 uppercase tracking-wider font-heading">Partnered Companies</p>
              <p className="text-xs text-slate-400">From emerging startups to enterprise platforms</p>
            </div>
          </div>
        </div>
      </div>
