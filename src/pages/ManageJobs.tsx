      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        {!isLoading && jobs.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {/* Stat Box 1 */}
            <div className="glass-card rounded-2xl p-6 border border-slate-200/80 shadow-md flex flex-col justify-between">
              <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Total Listings</span>
              <div className="mt-4.5">
                <span className="text-4xl font-black text-slate-900 font-heading">{jobs.length}</span>
            ...
            {/* Stat Box 2 */}
            <div className="glass-card rounded-2xl p-6 border border-slate-200/80 shadow-md flex flex-col justify-between">
              <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Mock Applications</span>
              <div className="mt-4.5">
            ...
            <div className="glass-card rounded-2xl p-5 border border-slate-200/80 shadow-md md:col-span-1">
              <span className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-3">Category Distribution</span>
              <div className="h-28">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={categoryStats}>
                    ...
                    <Tooltip 
                      contentStyle={{ background: '#ffffff', border: '1px solid #e2e8f0', borderRadius: '12px', fontSize: '11px', color: '#0f172a', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20 glass-card border border-slate-200/80 rounded-3xl space-y-4 shadow-md">
            <p className="text-slate-900 font-bold text-lg font-heading">No listings posted yet</p>
            <p className="text-xs text-slate-500 max-w-xs mx-auto">Create details maps and post your first career opening on JobNest.</p>
            <Link
              to="/jobs/add"
              className="inline-flex items-center gap-1.5 bg-gradient-to-r from-[#6C5CE7] to-[#00D2D3] text-white px-5 py-2.5 rounded-xl text-xs font-bold shadow-md shadow-[#6C5CE7]/20 hover:opacity-95 transition-all cursor-pointer"
            >
              Post First Job
            </Link>
          </div>
        ) : (
          <div className="glass-card border border-slate-200/80 rounded-3xl overflow-hidden shadow-lg">
            {/* Desktop Table View */}
                  ...
                  {jobs.map((job) => (
                    <tr key={job._id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="py-5 px-6 font-medium text-slate-600">
                        <span className="px-2.5 py-1 rounded-md bg-white border border-slate-200 text-xs shadow-sm">
                          {job.category}
                        </span>
                      </td>
                      <td className="py-5 px-6">
                      ...
                      <td className="py-5 px-6 text-right">
                        <div className="flex items-center justify-end gap-2.5">
                          ...
                          <button
                            ...
                            className="p-2 rounded-xl bg-white hover:bg-rose-50 text-rose-600 hover:text-rose-700 border border-slate-200 hover:border-rose-200 transition-all shadow-sm disabled:opacity-50 cursor-pointer"
                            title="Delete Job"
                          >
                ...
                <div key={job._id} className="p-6 space-y-4">
                  ...
                  <div className="flex items-center justify-between border-t border-slate-100 pt-4">
                    ...
                    <div className="flex items-center gap-2">
                      <Link
                        to={`/jobs/${job._id}`}
                        className="flex items-center gap-1 px-3 py-1.5 rounded-xl bg-white border border-slate-200 text-xs font-semibold text-slate-700 hover:text-slate-900 shadow-sm"
                      >
                      ...
                      <button
                        onClick={() => handleDelete(job._id, job.title)}
                        disabled={deletingId === job._id}
                        className="flex items-center gap-1 px-3 py-1.5 rounded-xl bg-rose-50 border border-rose-200 text-xs font-semibold text-rose-600 disabled:opacity-50 cursor-pointer"
                      >
