  ...
  return (
    <div className="min-h-screen bg-[#0f0f1a] pt-28 pb-20 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gradient-to-r from-[#6C5CE7]/10 to-[#00D2D3]/15 border border-white/5 p-6 rounded-3xl relative overflow-hidden">
          <div className="space-y-1 relative z-10">
            <h1 className="text-2xl font-bold font-heading text-slate-100">Explore Careers</h1>
            <p className="text-xs text-slate-400">Discover and apply to verify professional opportunities.</p>
          </div>
          
          <form onSubmit={handleSearchSubmit} className="flex gap-2.5 max-w-md w-full relative z-10 shrink-0">
            <div className="flex items-center bg-white/5 border border-white/10 focus-within:border-[#6C5CE7]/40 rounded-xl px-3 py-2 w-full">
            ...
            <button
              type="submit"
              className="bg-[#6C5CE7] hover:bg-[#5a3fd9] text-white px-4 py-2 rounded-xl text-sm font-semibold transition-all shrink-0"
            >
              Search
            </button>
          </form>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 items-start">
          ...
          <aside className="hidden lg:block p-6 rounded-2xl glass border border-white/5 space-y-6">
            ...
            <div className="space-y-2">
              ...
              <select
                ...
                className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-xs text-slate-300 outline-none focus:border-[#6C5CE7]/40"
              >
          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            <div className="flex justify-between items-center bg-white border border-slate-100 rounded-2xl px-5 py-3.5 shadow-sm">
              <span className="text-xs text-slate-500 font-medium">
                Showing <strong className="text-slate-800">{totalJobsCount}</strong> jobs
              </span>

              <div className="flex items-center gap-3">
                <button
                  onClick={() => setShowMobileFilters(true)}
                  className="lg:hidden flex items-center gap-1.5 text-xs bg-white border border-slate-200 px-3.5 py-2 rounded-xl text-slate-600 hover:text-slate-900"
                >
                ...
                <select
                  ...
                  className="bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-700 outline-none focus:border-[#6C5CE7]"
                >
                  <option value="newest" className="bg-white">Newest First</option>
                  <option value="oldest" className="bg-white">Oldest First</option>
                  <option value="salary_high" className="bg-white">Salary: High to Low</option>
                  <option value="salary_low" className="bg-white">Salary: Low to High</option>
                </select>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
              {isLoading ? (
                ...
                <div className="col-span-full py-16 text-center space-y-2 bg-white rounded-3xl border border-slate-100 shadow-sm">
                  <p className="text-slate-700 font-bold font-heading">No listings matches found</p>
                  ...
                  <button
                    onClick={handleClearFilters}
                    className="px-4 py-2 mt-4 text-xs font-semibold rounded-xl bg-slate-50 border border-slate-200 text-slate-600 hover:bg-slate-100"
                  >
                    Reset All Filters
                  </button>
                </div>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-3 pt-6 border-t border-slate-100">
                <button
                  ...
                  className="p-2.5 rounded-xl border border-slate-200 bg-white text-slate-500 hover:text-slate-800 disabled:opacity-30 disabled:cursor-not-allowed transition-all shadow-sm"
                >
                  ...
                  return (
                    <button
                      ...
                      className={`w-9 h-9 rounded-xl border text-xs font-bold transition-all ${
                        page === pNum
                          ? 'bg-[#6C5CE7] border-[#6C5CE7] text-white shadow-md shadow-[#6C5CE7]/20'
                ...
                <button
                  ...
                  onClick={() => updateUrlParams({ page: page + 1 })}
                  className="p-2.5 rounded-xl border border-white/10 bg-white/5 text-slate-400 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed transition-all"
                >
